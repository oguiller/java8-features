 ...
 *
 * it is extremely inefficient. The worst case, when you want to sort a list that is already sorted in reverse order,
 * is a performance of O(n2): For each iteration, you are only switching one element. The best case is when a list is
 * already sorted: You make one pass through the list, and because you have not switched any elements, you can stop.
